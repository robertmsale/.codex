...
pub fn stable_execute_code_contract() -> String {
    "Evaluate Starlark in the experimental host runtime. Use print(...) to emit final tool output; it accepts zero or more positional values, joins multiple values with one ASCII space, and appends one visible output line per call. Host calls return script values and do not implicitly become final output. Full command/process/script output is stored as durable output artifacts; default tool results expose bounded envelopes with artifact ids, byte/line counts, truncation metadata, preview, and tail only. Retrieve stored output intentionally with bounded helpers: outputs.last(), outputs.head(id, lines=N), outputs.tail(id, lines=N), outputs.slice(id, start_line=A, end_line=B), outputs.search(id, pattern, context=N), and outputs.stats(id). Starter-kit native APIs accept CWD-relative paths resolved under the session execution root, reject escapes and .git internals, and record audit rows. File/tree APIs: fs.read(path), fs.write(path, content, description), file.head(path, lines=N), file.tail(path, lines=N), file.read_lines(path, start, end), file.line_count(path), file.search(path, pattern, context=N), file.replace_exact(path, old, new, description), tree.list(path=\".\", depth=N), tree.find(path=\".\", name_glob=\"*.rs\", type=\"file\", max_results=N). tree.list returns compact display-oriented native tree plaintext with noise directories omitted by default; full plaintext is stored as a durable read artifact when the renderer compacts visible output. Mutation descriptions are required and must describe the concrete intended mutation. Safe git APIs are git.status(), git.diff(paths), git.restore(paths), git.add(paths), and git.commit(message, paths). Server APIs are server.start(action, args, name=\"\"), server.status(handle), server.url(handle), server.wait_ready(handle, timeout_ms=N), server.logs(handle, stream=\"stdout\", lines=N), and server.stop(handle); PORT is allocated by the runtime. Image APIs are image.capture_from_file(path, description) and image.describe(id); binary image bytes stay outside model-visible transcript text. Missing tooling must use tooling.request(title, need, attempted, proposed=\"\", urgency=\"normal\") so Rust records a typed routed packet. On failed execute_code results, run print(workflow_memory.help()) to search prior successful patterns for the last failed script. Workflow memory APIs: workflow_memory.help(), workflow_memory.remember_when(condition, title, reason), workflow_memory.mark_attempted(id, variant=True), and workflow_memory.mark_not_helpful(id, reason). Registered commands are accessed through cmd[\"object\"].method(...).sync() for synchronous execution or .start() for a managed async process handle. Inspect available registered commands with cmd.describe(), cmd[\"object\"].describe(), and cmd[\"object\"].method.describe() before using command-specific details. Use proc[handle].is_running(), await_for(mins=N), flush_buffer(), terminate(), and input(text) for managed processes. Handles are opaque session-only values, not OS PIDs. No raw shell, network, arbitrary environment, unregistered binaries, or undocumented command surfaces are available.".to_string()
}
...
mod cache_stable_discovery_tests {
    ...
    fn execute_code_contract_is_stable_while_command_context_changes() {
        ...
        let contract = stable_execute_code_contract();
        assert_eq!(contract, stable_execute_code_contract());
        assert!(contract.contains("Use print(...) to emit final tool output"));
        assert!(contract.contains("print(workflow_memory.help())"));
        assert!(!contract.contains("Use output(value)"));
