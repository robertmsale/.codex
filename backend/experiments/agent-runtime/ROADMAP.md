...
- Starlark lifecycle orchestration primitives:
  `crates/robdex-agent-runtime/src/lifecycle_hooks.rs`
- Project runtime seed workflows:
  `project-runtime-seeds/requirements_review.star`,
  `project-runtime-seeds/simulator_stewardship.star`
...
Starlark lifecycle orchestration is the generic workflow system for the experimental Agent Runtime: project runtime configs, hook bindings, typed packets/envelopes, hidden generic subagents, generic contracts/progress, resource leases, structured-output schema evidence, compaction summaries, selected-session projection, and typed Rust/Rinf runtime-config operations are Rust/PostgreSQL owned. Requirements Review is migrated to the hook-defined contract workflow surface: the product API/CLI/GUI names remain, while source/reviewer schemas, claim/verdict packets, reviewer subagents, routing, and progress updates are represented through the generic primitives. Role Admin UI structured editor is implemented as a full-screen Role Manager. Execution output persists stdout and stderr as separate durable artifacts; same-turn steering transcripts carry bounded explicit output plus metadata/artifact handles only. Workflow Memory inspection plus feedback is implemented inside Agent Runtime modal operations surfaces. iCloud remote profile discovery and document-import remote profile UX use Rust-owned typed transport boundaries. The connected Agent Runtime UI remains the canonical Robdex Workbench pattern with typed Rinf operations and must not regress to a dashboard shell, permanent operations pane, or diagnostics-first interaction model. Remaining gates are mDNS/Bonjour discovery, native iOS file-picker polish beyond the typed import boundary, broader execution expansion after GUI/runtime lifecycle is stable, workflow-memory editing/curation if the owner explicitly scopes it later, and any owner-approved production Robdex integration. Tokenizer-based accounting remains intentionally out of scope.
