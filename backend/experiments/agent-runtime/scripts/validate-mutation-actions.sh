...
DENY=$(cargo run --quiet -- new-session --role runtime-no-rg)
APPROVE=$(cargo run --quiet -- new-session --role runtime-approval-rg)
printf '\n[sessions]\nALLOW=%s\nDENY=%s\nAPPROVE=%s\nROOT=%s\n' "$ALLOW" "$DENY" "$APPROVE" "$ROOT"

run cargo run --quiet -- --workdir "$ROOT" send --session "$ALLOW" --message 'Use execute_code with exactly this Starlark source: fs.write("notes.txt", "hello mutation"); output("wrote")'
...
set +e
OUTSIDE=$(cargo run --quiet -- --workdir "$ROOT" send --session "$ALLOW" --message 'Use execute_code with exactly this Starlark source: fs.write("../outside.txt", "bad"); output("bad")' 2>&1)
OUTSIDE_STATUS=$?
set -e
printf 'outside_root_status=%s\n' "$OUTSIDE_STATUS"
printf '%s\n' "$OUTSIDE" | rg 'escapes execution root|parent is not accessible|failed'

mkdir -p "$ROOT/.git"
set +e
GITWRITE=$(cargo run --quiet -- --workdir "$ROOT" send --session "$ALLOW" --message 'Use execute_code with exactly this Starlark source: fs.write(".git/config", "bad"); output("bad")' 2>&1)
GITWRITE_STATUS=$?
set -e
printf 'git_write_status=%s\n' "$GITWRITE_STATUS"
printf '%s\n' "$GITWRITE" | rg 'git internals|failed'

run cargo run --quiet -- --workdir "$ROOT" send --session "$DENY" --message 'Use execute_code with exactly this Starlark source: fs.write("denied.txt", "nope"); output("denied")'
...
run cargo run --quiet -- --workdir "$ROOT" send --session "$APPROVE" --message 'Use execute_code with exactly this Starlark source: fs.write("approved.txt", "stored content"); output("paused")'
...
run cargo run --quiet -- approvals decide "$APPROVAL_ID" --decision approved --reason 'mutation validation approval'
...
+delta
 gamma'
run cargo run --quiet -- --workdir "$ROOT" send --session "$ALLOW" --message "Use execute_code with exactly this Starlark source: patch.apply('''$PATCH'''); output('patched')"
...
-two
+blocked'
run cargo run --quiet -- --workdir "$ROOT" send --session "$DENY" --message "Use execute_code with exactly this Starlark source: patch.apply('''$DENY_PATCH'''); output('denied patch')"
...
-blue
+green'
run cargo run --quiet -- --workdir "$ROOT" send --session "$APPROVE" --message "Use execute_code with exactly this Starlark source: patch.apply('''$APPROVAL_PATCH'''); output('approval patch')"
...
run cargo run --quiet -- approvals decide "$PATCH_APPROVAL_ID" --decision approved --reason 'patch validation approval'
printf '\nafter_decide_patch_content='; cat "$ROOT/patch-approval.txt"
run cargo run --quiet -- approvals resume "$PATCH_APPROVAL_ID"
...
-right
+wrong'
run cargo run --quiet -- --workdir "$ROOT" send --session "$APPROVE" --message "Use execute_code with exactly this Starlark source: patch.apply('''$FAIL_PATCH'''); output('approval fail patch')"
...
run cargo run --quiet -- approvals decide "$FAIL_PATCH_APPROVAL_ID" --decision approved --reason 'patch failure validation approval'
set +e
FAIL_RESUME_OUT=$(cargo run --quiet -- approvals resume "$FAIL_PATCH_APPROVAL_ID" 2>&1)
FAIL_RESUME_STATUS=$?
set -e
...
+b'
set +e
BADPATCH_OUT=$(cargo run --quiet -- --workdir "$ROOT" send --session "$ALLOW" --message "Use execute_code with exactly this Starlark source: patch.apply('''$BADPATCH'''); output('bad')" 2>&1)
BADPATCH_STATUS=$?
set -e
printf 'bad_patch_status=%s\n' "$BADPATCH_STATUS"
printf '%s\n' "$BADPATCH_OUT" | rg 'git internals|failed'

run cargo run --quiet -- --workdir /Users/robertsale/.codex/backend/experiments/agent-runtime send --session "$ALLOW" --message 'Use execute_code with exactly this Starlark source: s = cmd["git"].status(); d = cmd["git"].diff(args=[]); output(s + d)'
...
run cargo run --quiet -- --workdir /Users/robertsale/.codex/backend/experiments/agent-runtime send --session "$ALLOW" --message 'Use execute_code with exactly this Starlark source: result = cmd["cargo"].check(args=[]); output("cargo checked")'
