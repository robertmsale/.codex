...
run cargo run --quiet --bin robdex-agent-runtime -- send --session "$ALLOW" --message 'Use execute_code with exactly this Starlark source: fs.write("notes.txt", "hello mutation"); print("wrote")'
...
set +e
OUTSIDE=$(cargo run --quiet --bin robdex-agent-runtime -- send --session "$ALLOW" --message 'Use execute_code with exactly this Starlark source: fs.write("../outside.txt", "bad"); print("bad")' 2>&1)
OUTSIDE_STATUS=$?
set -e
...
mkdir -p "$ROOT/.git"
set +e
GITWRITE=$(cargo run --quiet --bin robdex-agent-runtime -- send --session "$ALLOW" --message 'Use execute_code with exactly this Starlark source: fs.write(".git/config", "bad"); print("bad")' 2>&1)
GITWRITE_STATUS=$?
set -e
...
run cargo run --quiet --bin robdex-agent-runtime -- send --session "$DENY" --message 'Use execute_code with exactly this Starlark source: fs.write("denied.txt", "nope"); print("denied")'
...
run cargo run --quiet --bin robdex-agent-runtime -- send --session "$APPROVE" --message 'Use execute_code with exactly this Starlark source: fs.write("approved.txt", "stored content"); print("paused")'
...
+delta
 gamma'
run cargo run --quiet --bin robdex-agent-runtime -- send --session "$ALLOW" --message "Use execute_code with exactly this Starlark source: patch.apply('''$PATCH'''); print('patched')"
...
-two
+blocked'
run cargo run --quiet --bin robdex-agent-runtime -- send --session "$DENY" --message "Use execute_code with exactly this Starlark source: patch.apply('''$DENY_PATCH'''); print('denied patch')"
...
-blue
+green'
run cargo run --quiet --bin robdex-agent-runtime -- send --session "$APPROVE" --message "Use execute_code with exactly this Starlark source: patch.apply('''$APPROVAL_PATCH'''); print('approval patch')"
...
-right
+wrong'
run cargo run --quiet --bin robdex-agent-runtime -- send --session "$APPROVE" --message "Use execute_code with exactly this Starlark source: patch.apply('''$FAIL_PATCH'''); print('approval fail patch')"
...
+b'
set +e
BADPATCH_OUT=$(cargo run --quiet --bin robdex-agent-runtime -- send --session "$ALLOW" --message "Use execute_code with exactly this Starlark source: patch.apply('''$BADPATCH'''); print('bad')" 2>&1)
BADPATCH_STATUS=$?
set -e
printf 'bad_patch_status=%s\n' "$BADPATCH_STATUS"
printf '%s\n' "$BADPATCH_OUT" | rg 'git internals|failed'

run cargo run --quiet --bin robdex-agent-runtime -- send --session "$ALLOW" --message 'Use execute_code with exactly this Starlark source: s = cmd["git"].status(); d = cmd["git"].diff(args=[]); print(s + d)'
...
run cargo run --quiet --bin robdex-agent-runtime -- send --session "$ALLOW" --message 'Use execute_code with exactly this Starlark source: result = cmd["cargo"].check(args=[]); print("cargo checked")'
