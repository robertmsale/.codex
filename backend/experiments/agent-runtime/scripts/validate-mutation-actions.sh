...
ALLOW=$(cargo run --quiet -- sessions new --role runtime-allow --workdir "$ROOT")
DENY=$(cargo run --quiet -- sessions new --role runtime-no-rg --workdir "$ROOT")
APPROVE=$(cargo run --quiet -- sessions new --role runtime-approval-rg --workdir "$ROOT")
printf '\n[sessions]\nALLOW=%s\nDENY=%s\nAPPROVE=%s\nROOT=%s\n' "$ALLOW" "$DENY" "$APPROVE" "$ROOT"

run cargo run --quiet -- send --session "$ALLOW" --message 'Use execute_code with exactly this Starlark source: fs.write("notes.txt", "hello mutation"); output("wrote")'
...
set +e
OUTSIDE=$(cargo run --quiet -- send --session "$ALLOW" --message 'Use execute_code with exactly this Starlark source: fs.write("../outside.txt", "bad"); output("bad")' 2>&1)
OUTSIDE_STATUS=$?
set -e
...
mkdir -p "$ROOT/.git"
set +e
GITWRITE=$(cargo run --quiet -- send --session "$ALLOW" --message 'Use execute_code with exactly this Starlark source: fs.write(".git/config", "bad"); output("bad")' 2>&1)
GITWRITE_STATUS=$?
set -e
...
run cargo run --quiet -- send --session "$DENY" --message 'Use execute_code with exactly this Starlark source: fs.write("denied.txt", "nope"); output("denied")'
...
run cargo run --quiet -- send --session "$APPROVE" --message 'Use execute_code with exactly this Starlark source: fs.write("approved.txt", "stored content"); output("paused")'
...
+delta
 gamma'
run cargo run --quiet -- send --session "$ALLOW" --message "Use execute_code with exactly this Starlark source: patch.apply('''$PATCH'''); output('patched')"
...
-two
+blocked'
run cargo run --quiet -- send --session "$DENY" --message "Use execute_code with exactly this Starlark source: patch.apply('''$DENY_PATCH'''); output('denied patch')"
...
-blue
+green'
run cargo run --quiet -- send --session "$APPROVE" --message "Use execute_code with exactly this Starlark source: patch.apply('''$APPROVAL_PATCH'''); output('approval patch')"
...
-right
+wrong'
run cargo run --quiet -- send --session "$APPROVE" --message "Use execute_code with exactly this Starlark source: patch.apply('''$FAIL_PATCH'''); output('approval fail patch')"
...
+b'
set +e
BADPATCH_OUT=$(cargo run --quiet -- send --session "$ALLOW" --message "Use execute_code with exactly this Starlark source: patch.apply('''$BADPATCH'''); output('bad')" 2>&1)
BADPATCH_STATUS=$?
set -e
printf 'bad_patch_status=%s\n' "$BADPATCH_STATUS"
printf '%s\n' "$BADPATCH_OUT" | rg 'git internals|failed'

run cargo run --quiet -- send --session "$ALLOW" --message 'Use execute_code with exactly this Starlark source: s = cmd["git"].status(); d = cmd["git"].diff(args=[]); output(s + d)'
...
run cargo run --quiet -- send --session "$ALLOW" --message 'Use execute_code with exactly this Starlark source: result = cmd["cargo"].check(args=[]); output("cargo checked")'
