...
run cargo run --quiet --bin robdex-agent-runtime -- roles validate --manifest "$TMPDIR/runtime-admin.json"
run cargo run --quiet --bin robdex-agent-runtime -- roles create "$TMPDIR/runtime-admin.json"
CREATE_VERSION_COUNT_BEFORE="$(sql "select count(*) from role_versions where role_id='runtime-admin'")"
set +e
DUP_CREATE="$(cargo run --quiet --bin robdex-agent-runtime -- roles create "$TMPDIR/runtime-admin.json" 2>&1)"
DUP_CREATE_STATUS=$?
set -e
...
LIST_OUT="$(cargo run --quiet --bin robdex-agent-runtime -- roles list)"
printf '%s\n' "$LIST_OUT" | rg '"id":"runtime-admin"'
SHOW_V1="$(cargo run --quiet --bin robdex-agent-runtime -- roles show runtime-admin)"
printf '%s\n' "$SHOW_V1" | rg '"instructionText"'
OLD_SESSION="$(cargo run --quiet --bin robdex-agent-runtime -- sessions new --role runtime-admin)"
...
run cargo run --quiet --bin robdex-agent-runtime -- roles update "$TMPDIR/runtime-admin.json"
...
PY
set +e
MISSING_UPDATE="$(cargo run --quiet --bin robdex-agent-runtime -- roles update "$TMPDIR/runtime-missing-update.json" 2>&1)"
...
NEW_SESSION="$(cargo run --quiet --bin robdex-agent-runtime -- sessions new --role runtime-admin)"
...
VERSIONS_JSON="$(cargo run --quiet --bin robdex-agent-runtime -- roles versions runtime-admin)"
...
run cargo run --quiet --bin robdex-agent-runtime -- roles version "$V1_ID"
run cargo run --quiet --bin robdex-agent-runtime -- roles activate runtime-admin --version-id "$V1_ID"
ROLLBACK_SESSION="$(cargo run --quiet --bin robdex-agent-runtime -- sessions new --role runtime-admin)"
...
run cargo run --quiet --bin robdex-agent-runtime -- roles activate runtime-admin --version-id "$V2_ID"
run cargo run --quiet --bin robdex-agent-runtime -- roles export runtime-admin --out "$EXPORT_FILE"
test -s "$EXPORT_FILE"
rm -rf "$TMPDIR/prompts"
run cargo run --quiet --bin robdex-agent-runtime -- roles import "$EXPORT_FILE"
IMPORT_COUNT="$(sql "select count(*) from role_versions where role_id='runtime-admin'")"
[[ "$IMPORT_COUNT" -ge 3 ]]

run cargo run --quiet --bin robdex-agent-runtime -- roles archive runtime-admin
ARCHIVED_STATUS="$(sql "select status from roles where id='runtime-admin'")"
assert_eq archived_status "archived" "$ARCHIVED_STATUS"
set +e
ARCHIVED_NEW="$(cargo run --quiet --bin robdex-agent-runtime -- sessions new --role runtime-admin 2>&1)"
ARCHIVED_NEW_STATUS=$?
set -e
printf 'archived_new_status=%s\n%s\n' "$ARCHIVED_NEW_STATUS" "$ARCHIVED_NEW"
printf '%s\n' "$ARCHIVED_NEW" | rg 'no rows returned|not found'
run cargo run --quiet --bin robdex-agent-runtime -- roles unarchive runtime-admin
...
UNARCHIVED_SESSION="$(cargo run --quiet --bin robdex-agent-runtime -- sessions new --role runtime-admin)"
...
for mode in invalid_json empty_prompt invalid_id missing_model unknown_action concrete_cmd cap_mismatch bad_routing_mode bad_reserved bad_recipient; do
  ...
  set +e
  OUT="$(cargo run --quiet --bin robdex-agent-runtime -- roles validate --manifest "$bad" 2>&1)"
  STATUS=$?
  set -e
...
run cargo run --quiet --bin robdex-agent-runtime -- roles show runtime-admin
run cargo run --quiet --bin robdex-agent-runtime -- roles versions runtime-admin
run cargo run --quiet --bin robdex-agent-runtime -- roles version "$V2_ID"
