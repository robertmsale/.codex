...
run cargo run --quiet --bin robdex-agent-runtime -- command-registry requests decide --session "$ADMIN_SESSION" "$REGISTRY_REQUEST" --status approved --final-scope global --final-policy ownerApproval --final-command-file /tmp/agent-runtime-resume-rg.json
run cargo run --quiet --bin robdex-agent-runtime -- command-registry requests apply --session "$ADMIN_SESSION" "$REGISTRY_REQUEST"

APPROVE_SESSION=$(cargo run --quiet --bin robdex-agent-runtime -- sessions new --role runtime-allow)
PENDING_SESSION=$(cargo run --quiet --bin robdex-agent-runtime -- sessions new --role runtime-allow)
...
printf '\n[approvals show linked paused action]\n'; cargo run --quiet --bin robdex-agent-runtime -- approvals show "$APPROVAL_ID" | python3 -c 'import json,sys; d=json.load(sys.stdin); p=d["pausedActions"][0]; print(d["status"], p["status"], p["actionName"], p["actionInput"]["argv"], p["actionInput"]["cwd"])'

run cargo run --quiet --bin robdex-agent-runtime -- approvals decide "$APPROVAL_ID" --decision approved --reason 'validation approval'
...
set +e
SECOND=$(cargo run --quiet --bin robdex-agent-runtime -- approvals resume "$APPROVAL_ID" 2>&1)
SECOND_STATUS=$?
set -e
...
run cargo run --quiet --bin robdex-agent-runtime -- approvals decide "$ASYNC_APPROVAL" --decision approved --reason 'validation async approval'
run cargo run --quiet --bin robdex-agent-runtime -- approvals resume "$ASYNC_APPROVAL"
...
set +e
DENIED_OUT=$(cargo run --quiet --bin robdex-agent-runtime -- approvals resume "$DENIED_APPROVAL" 2>&1)
DENIED_STATUS=$?
set -e
