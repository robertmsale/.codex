...
ADMIN=$(cargo run --quiet -- sessions new --role runtime-allow --project ergonomics --workdir .)
printf 'admin_session=%s\n' "$ADMIN"

run cargo run --quiet -- command-registry seed-requests --session "$ADMIN" --mode refresh
...
REVIEW_BEFORE=$(cargo run --quiet -- command-registry requests review "$REQUEST_ID")
...
run cargo run --quiet -- command-registry requests final-template "$REQUEST_ID" --out "$TEMPLATE"
test -s "$TEMPLATE"
python3 - "$TEMPLATE" <<'PY'
import json, sys
path = sys.argv[1]
...
set +e
MISSING_SCOPE=$(cargo run --quiet -- command-registry requests preview-decision "$REQUEST_ID" --status approved --final-policy allow --final-command-file "$TEMPLATE" 2>&1)
...
set +e
MISSING_POLICY=$(cargo run --quiet -- command-registry requests preview-decision "$REQUEST_ID" --status approved --final-scope global --final-command-file "$TEMPLATE" 2>&1)
...
PY
set +e
INVALID_OUT=$(cargo run --quiet -- command-registry requests preview-decision "$REQUEST_ID" --status approved --final-scope global --final-policy allow --final-command-file "$INVALID" 2>&1)
INVALID_STATUS=$?
set -e
printf 'invalid_status=%s\n%s\n' "$INVALID_STATUS" "$INVALID_OUT"
printf '%s\n' "$INVALID_OUT" | rg 'registry command action'

PREVIEW=$(cargo run --quiet -- command-registry requests preview-decision "$REQUEST_ID" --status approved --final-scope global --final-policy allow --final-command-file "$TEMPLATE")
...
run cargo run --quiet -- command-registry requests decide --session "$ADMIN" "$REQUEST_ID" --status approved --final-scope global --final-policy allow --final-command-file "$TEMPLATE"
REVIEW_AFTER=$(cargo run --quiet -- command-registry requests review "$REQUEST_ID")
...
run cargo run --quiet -- command-registry requests apply --session "$ADMIN" "$REQUEST_ID"
...
run cargo run --quiet -- command-registry requests decide --session "$ADMIN" "$DENIED_ID" --status denied
set +e
DENIED_APPLY=$(cargo run --quiet -- command-registry requests apply --session "$ADMIN" "$DENIED_ID" 2>&1)
DENIED_APPLY_STATUS=$?
set -e
...
run cargo run --quiet -- command-registry requests decide --session "$ADMIN" "$PROJECT_REQ" --status approved --final-scope project --final-project ergonomics --final-policy allow --final-command-file "$PROJECT_COMMAND"
...
set +e
CONFLICT_OUT=$(cargo run --quiet -- command-registry requests preview-decision "$GLOBAL_CONFLICT_REQ" --status approved --final-scope global --final-policy allow --final-command-file "$PROJECT_COMMAND" 2>&1)
CONFLICT_STATUS=$?
set -e
...
APPROVAL_SHOW=$(cargo run --quiet -- approvals show "$APPROVAL_ID")
