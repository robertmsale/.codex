...
seed={"actionId":action,"binaryName":"rg","candidatePaths":["/opt/homebrew/bin/rg","/usr/local/bin/rg","/usr/bin/rg"],"starlarkObject":obj,"starlarkMethod":"run","argvPrefix":["--files"],"allowArgsArg":True,"allowCwdArg":True,"defaultCwd":".","cwdPolicy":"underExecutionRoot","envPolicy":"empty","timeoutMs":5000,"outputLimitBytes":12000,"mutationClass":"readOnly","modelDescription":f"scoped validation helper {action}","forbiddenArgs":[]}
...
approve() {
  ...
  if [[ "$scope" == "project" ]]; then
    run cargo run --quiet -- command-registry requests decide --session "$ADMIN" "$id" --status approved --final-scope project --final-project "$project" --final-policy "$policy" --final-command-file "$seed_file"
  else
    run cargo run --quiet -- command-registry requests decide --session "$ADMIN" "$id" --status approved --final-scope global --final-policy "$policy" --final-command-file "$seed_file"
  fi
}

run cargo run --quiet -- init-db
run cargo run --quiet -- roles import-seeds
ADMIN=$(cargo run --quiet -- new-session --role runtime-allow --project admin)
printf 'admin_session=%s\n' "$ADMIN"

write_seed /tmp/scoped-native-seed.json cmd.rg.native_request rg_native_request
NATIVE_SESSION=$(cargo run --quiet -- new-session --role runtime-allow --project alpha)
run cargo run --quiet -- --workdir . send --session "$NATIVE_SESSION" --message 'Call request_command_registry_change exactly once. Use operation add. proposedCommand actionId cmd.rg.native_request, binaryName rg, candidatePaths ["/opt/homebrew/bin/rg","/usr/local/bin/rg","/usr/bin/rg"], starlarkObject rg_native_request, starlarkMethod run, argvPrefix ["--files"], defaultCwd ".", cwdPolicy "underExecutionRoot", envPolicy "empty", timeoutMs 5000, outputLimitBytes 12000, mutationClass "readOnly", modelDescription "native request validation helper", allowCwdArg true, allowArgsArg true, forbiddenArgs []. rationale "need a native registry request validation command". intendedUse "validate native request tool". currentBlockerOrNeed "missing command surface". requesterContext sourceRole "runtime-allow", sourceTask "validation", observedError "missing command", neededFor "scoped request validation".'
...
run cargo run --quiet -- command-registry requests decide --session "$ADMIN" "$NATIVE_REQ" --status denied
...
GLOBAL_REQ=$(cargo run --quiet -- command-registry requests create --session "$ADMIN" /tmp/scoped-global-request.json)
MISSING_FINAL=$(cargo run --quiet -- command-registry requests decide --session "$ADMIN" "$GLOBAL_REQ" --status approved 2>&1 || true)
...
run cargo run --quiet -- command-registry requests apply --session "$ADMIN" "$GLOBAL_REQ"
GLOBAL_SESSION=$(cargo run --quiet -- new-session --role runtime-allow --project beta)
run cargo run --quiet -- --workdir . send --session "$GLOBAL_SESSION" --message 'Use execute_code with exactly this Starlark source: files = cmd["rg_global_visible"].run(args=["-g", "Cargo.toml"], cwd="."); output(files)'
...
GLOBAL_DENY_REQ=$(cargo run --quiet -- command-registry requests create --session "$ADMIN" /tmp/scoped-global-deny-request.json)
approve "$GLOBAL_DENY_REQ" global '' deny /tmp/scoped-global-deny-seed.json
run cargo run --quiet -- command-registry requests apply --session "$ADMIN" "$GLOBAL_DENY_REQ"
GLOBAL_DENY_SESSION=$(cargo run --quiet -- new-session --role runtime-allow --project beta)
GLOBAL_DENY_OUT=$(cargo run --quiet -- --workdir . send --session "$GLOBAL_DENY_SESSION" --message 'Use execute_code with exactly this Starlark source: files = cmd["rg_global_denied"].run(args=["-g", "Cargo.toml"], cwd="."); output(files)' 2>&1 || true)
printf 'global_deny_attempt=%s\n' "$GLOBAL_DENY_OUT"
printf 'global_deny_visible_contract='; sql "select count(*) from event_stream where session_id='$GLOBAL_DENY_SESSION' and event_type='model.tool_call' and payload->'request'->>'executeCodeContract' like '%rg_global_denied%'"
[[ "$(sql "select count(*) from event_stream where session_id='$GLOBAL_DENY_SESSION' and event_type='model.tool_call' and payload->'request'->>'executeCodeContract' like '%rg_global_denied%'")" -gt 0 ]]
...
GLOBAL_ORCH_REQ=$(cargo run --quiet -- command-registry requests create --session "$ADMIN" /tmp/scoped-global-orch-request.json)
approve "$GLOBAL_ORCH_REQ" global '' orchestratorApproval /tmp/scoped-global-orch-seed.json
run cargo run --quiet -- command-registry requests apply --session "$ADMIN" "$GLOBAL_ORCH_REQ"
GLOBAL_ORCH_SESSION=$(cargo run --quiet -- new-session --role runtime-allow --project beta)
GLOBAL_ORCH_OUT=$(cargo run --quiet -- --workdir . send --session "$GLOBAL_ORCH_SESSION" --message 'Use execute_code with exactly this Starlark source: files = cmd["rg_global_orch_approval"].run(args=["-g", "Cargo.toml"], cwd="."); output(files)' 2>&1 || true)
...
PROJECT_REQ=$(cargo run --quiet -- command-registry requests create --session "$ADMIN" /tmp/scoped-project-request.json)
approve "$PROJECT_REQ" project alpha allow /tmp/scoped-project-seed.json
run cargo run --quiet -- command-registry requests apply --session "$ADMIN" "$PROJECT_REQ"
ALPHA_SESSION=$(cargo run --quiet -- new-session --role runtime-allow --project alpha)
BETA_SESSION=$(cargo run --quiet -- new-session --role runtime-allow --project beta)
run cargo run --quiet -- --workdir . send --session "$ALPHA_SESSION" --message 'Use execute_code with exactly this Starlark source: files = cmd["rg_project_visible"].run(args=["-g", "Cargo.toml"], cwd="."); output(files)'
BETA_OUT=$(cargo run --quiet -- --workdir . send --session "$BETA_SESSION" --message 'Use execute_code with exactly this Starlark source: files = cmd["rg_project_visible"].run(args=["-g", "Cargo.toml"], cwd="."); output(files)' 2>&1 || true)
...
PROJECT_OWNER_REQ=$(cargo run --quiet -- command-registry requests create --session "$ADMIN" /tmp/scoped-project-owner-request.json)
approve "$PROJECT_OWNER_REQ" project alpha ownerApproval /tmp/scoped-project-owner-seed.json
run cargo run --quiet -- command-registry requests apply --session "$ADMIN" "$PROJECT_OWNER_REQ"
PROJECT_OWNER_SESSION=$(cargo run --quiet -- new-session --role runtime-allow --project alpha)
PROJECT_OWNER_OUT=$(cargo run --quiet -- --workdir . send --session "$PROJECT_OWNER_SESSION" --message 'Use execute_code with exactly this Starlark source: files = cmd["rg_project_owner_approval"].run(args=["-g", "Cargo.toml"], cwd="."); output(files)' 2>&1 || true)
...
CONFLICT_REQ=$(cargo run --quiet -- command-registry requests create --session "$ADMIN" /tmp/scoped-conflict-request.json)
