...
if [[ "${ROBDEX_AGENT_RUNTIME_LIVE_SERVER_SMOKE:-}" != "1" ]]; then
  echo "Refusing live model smoke. Set ROBDEX_AGENT_RUNTIME_LIVE_SERVER_SMOKE=1." >&2
  exit 2
fi
...
ROLE_ID="${ROBDEX_AGENT_RUNTIME_LIVE_SERVER_ROLE:-runtime-live-server-gpt54mini}"
WORKDIR="${ROBDEX_AGENT_RUNTIME_LIVE_SERVER_WORKDIR:-$(pwd)}"
PROJECT="${ROBDEX_AGENT_RUNTIME_LIVE_SERVER_PROJECT:-agent-runtime-live-smoke}"
...
seed["id"] = role_id
seed["version"] = "1.0.54"
seed["displayName"] = "Runtime Live Server Smoke GPT 5.4 Mini"
seed["modelDefaults"]["model"] = "gpt-5.4-mini"
seed["modelDefaults"]["reasoningEffort"] = "low"
seed["instructionText"] = (
    "You are a live server smoke-test role. For the user's request, call only "
...
echo "Importing live smoke role $ROLE_ID with model gpt-5.4-mini"
...
session_json="$(curl -fsS -X POST "$BASE_URL/sessions" \
  ...
  -d "{\"role\":\"$ROLE_ID\",\"project\":\"$PROJECT\",\"workdir\":\"$WORKDIR\",\"title\":\"live server smoke gpt-5.4-mini\",\"name\":\"live-server-smoke\"}")"
...
send_json="$(curl -fsS -X POST "$BASE_URL/sessions/$session_id/send" \
  -H 'content-type: application/json' \
  -d '{"message":"Use execute_code with exactly this harmless read-only Starlark: content = fs.read(\"Cargo.toml\"); output({\"smoke\":\"ok\",\"contains_workspace\":\"workspace\" in content})"}')"
