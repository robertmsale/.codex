...
- `GET /roles/editor/options` returns Rust-owned editor metadata such as known actions, decision values, routing modes, and recipients. Recipient options use the same source of truth as routing validation: `owner` plus active DB-backed runtime role ids.
...
Routing metadata is structured role data. The supported mode is `direct`, with `defaultRecipient` and `allowedRecipients`. Valid recipients are exactly `owner` plus active DB-canonical role IDs. Seed import has a bounded import context so seeded role IDs can reference each other during validation, but magic strings such as `operator`, `orchestrator`, and `runtime` are rejected unless they are actual active runtime role IDs. Runtime routing or validation failures before a model request are projected as failed `Runtime` system rows, not Assistant-authored chat messages. Route evaluation records `route.decision`; no multi-agent message delivery is implemented in this phase.
