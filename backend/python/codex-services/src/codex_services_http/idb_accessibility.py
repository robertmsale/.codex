...
ORIENTATION_BEACON_ID = "automation.orientationBeacon"


def parse_orientation_beacon_value(value: Any) -> dict[str, Any] | None:
    ...
    if not parts:
        return None
    orientation = parts[0]
    if orientation not in {"portraitUp", "portraitDown", "landscapeLeft", "landscapeRight"}:
        return None
    payload: dict[str, Any] = {"orientation": orientation}
    if len(parts) >= 3:
        try:
            payload["width"] = int(float(parts[1]))
            payload["height"] = int(float(parts[2]))
        except Exception:
            pass
    return payload


def orientation_transform_for_value(orientation: str) -> str:
    mapping = {
        "portraitUp": "portrait_0",
        "portraitDown": "portrait_180",
        "landscapeRight": "landscape_90",
        "landscapeLeft": "landscape_270",
    }
    try:
        return mapping[orientation]
    except KeyError as error:
        raise AccessibilityError(f"Unsupported orientation beacon value: {orientation!r}.") from error


def find_orientation_beacon(elements: list[dict[str, Any]]) -> dict[str, Any] | None:
    for element in elements:
        unique_id = (element.get("AXUniqueId") or "").strip()
        label = (element.get("AXLabel") or "").strip()
        parsed = parse_orientation_beacon_value(element.get("AXValue"))
        if parsed and (unique_id == ORIENTATION_BEACON_ID or label == "orientation_beacon"):
            return element
    return None


def orientation_metadata_from_elements(elements: list[dict[str, Any]]) -> dict[str, Any] | None:
    beacon = find_orientation_beacon(elements)
    if beacon is None:
        return None
    parsed = parse_orientation_beacon_value(beacon.get("AXValue"))
    if parsed is None:
        return None
    parsed["transform"] = orientation_transform_for_value(str(parsed["orientation"]))
    return parsed
...
def transform_accessibility_point(
    *,
    point: tuple[int, int],
    portrait_width: int,
    portrait_height: int,
    ...
    raise AccessibilityError(f"Unsupported idb coordinate transform: {transform!r}.")
