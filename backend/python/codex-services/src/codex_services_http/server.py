...
app = FastAPI(title="codex-gitops-http", version="0.1.0")
PATHS = load_paths()
...
def _mutating_rebase_continue(args: dict[str, Any], paths: BridgePaths) -> str:
    ...
    _run_process_with_env(
        ...
        cwd=repo_path,
        paths=paths,
        error_label="git rebase --continue",
        extra_env={"GIT_EDITOR": "true", "EDITOR": "true", "VISUAL": "true"},
    )
    return sanitize_for_response("continued rebase", paths)
...
def _git_worktree_root(repo_path: Path) -> Path:
...
def _git_worktree_create(args: dict[str, Any], paths: BridgePaths) -> str:
    ...
    if not base_branch or not branch_name or not worktree_name:
        raise HTTPException(status_code=400, detail="repo_path, base_branch, branch_name, and worktree_name are required")

    repo_path = _require_local_git_repo(repo_path)
    worktree_root = _git_worktree_root(repo_path)
    worktree_root.mkdir(parents=True, exist_ok=True)
    worktree_path = (worktree_root / worktree_name).resolve()
    if worktree_path.exists():
        raise HTTPException(status_code=400, detail=f"Worktree path already exists: {worktree_path}")

    _run_process(
        ["git", "-C", str(repo_path), "fetch", "-q", "origin", "--prune"],
        cwd=repo_path,
        paths=paths,
        error_label="git fetch --prune",
    )
    _run_process(
        ["git", "-C", str(repo_path), "show-ref", "--verify", "--quiet", f"refs/remotes/origin/{base_branch}"],
        cwd=repo_path,
        paths=paths,
        error_label=f"resolve origin/{base_branch}",
    )
    ...
    if existing_branch.returncode == 0:
        raise HTTPException(status_code=400, detail=f"Local branch already exists: {branch_name}")
    ...
    _run_process(
        ...
        cwd=repo_path,
        paths=paths,
        error_label="git worktree add",
    )
    return sanitize_for_response(
        f"created worktree {worktree_path} on branch {branch_name} from origin/{base_branch}",
        paths,
    )
...
def _git_worktree_cleanup(args: dict[str, Any], paths: BridgePaths) -> str:
    ...
    checked_out_elsewhere = subprocess.run(
    ...
    if f"branch refs/heads/{branch}" not in checked_out_elsewhere:
        subprocess.run(
...
def _maybe_stash(repo_path: Path, paths: BridgePaths, label: str) -> tuple[str, bool]:
    ...
    if not status:
        return "", False
    stash_name = f"{label}-{int(time.time())}"
    _run_process(
        ["git", "-C", str(repo_path), "stash", "push", "-u", "-m", stash_name],
        cwd=repo_path,
        paths=paths,
        error_label="git stash push",
    )
    return stash_name, True


def _restore_stash(repo_path: Path, paths: BridgePaths, stash_name: str) -> str:
    if not stash_name:
        return ""
    stash_list = _run_process(
        ["git", "-C", str(repo_path), "stash", "list", "--format=%gd %s"],
        cwd=repo_path,
        paths=paths,
    ...
    for line in stash_list:
        if stash_name in line:
            ...
            break
    if not stash_ref:
        return f"stash {stash_name} was preserved for manual recovery"
    ...
    _run_process(
        ["git", "-C", str(repo_path), "stash", "drop", stash_ref],
        cwd=repo_path,
        paths=paths,
        error_label="git stash drop",
    )
    return f"restored stash {stash_name}"
...
def _git_publish_worktree(args: dict[str, Any], paths: BridgePaths) -> str:
    ...
    if push_process.returncode != 0:
        detail = (push_process.stderr or push_process.stdout or "git push failed").strip()
        raise HTTPException(status_code=400, detail=sanitize_for_response(detail, paths))
    ...
    if pr_view.returncode != 0:
        ...
        if create_process.returncode != 0:
            detail = (create_process.stderr or create_process.stdout or "gh pr create failed").strip()
            raise HTTPException(status_code=400, detail=sanitize_for_response(detail, paths))
        pr_view = subprocess.run(
            ["gh", "pr", "view", "--json", "number,url,state,isDraft,headRefName,baseRefName,title"],
            cwd=worktree_path,
    ...
    if pr_view.returncode != 0:
        detail = (pr_view.stderr or pr_view.stdout or "gh pr view failed").strip()
        raise HTTPException(status_code=400, detail=sanitize_for_response(detail, paths))

    try:
        pr_data = __import__("json").loads(pr_view.stdout or "{}")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=sanitize_for_response(f"failed to parse gh pr view output: {exc}", paths)) from exc

    publish_text = (
        f"published branch {pr_data.get('headRefName') or branch} "
        f"to {pr_data.get('baseRefName') or resolved_integration_branch}; "
        f"PR #{pr_data.get('number')} "
        f"({pr_data.get('url') or 'no-url'}) "
        f"state={pr_data.get('state') or 'unknown'} "
        f"draft={pr_data.get('isDraft')} "
    ...
    return sanitize_for_response(f"{publish_text}. refreshed remote-tracking refs.", paths)
...
def _git_fetch(args: dict[str, Any], paths: BridgePaths) -> str:
    repo_path = _require_local_git_repo(require_allowed_path(str(args.get("repo_path") or args.get("worktree_path") or ""), paths))
    remote = str(args.get("remote") or "origin").strip() or "origin"
    _run_process(
        ["git", "-C", str(repo_path), "fetch", "-q", remote, "--prune"],
        cwd=repo_path,
        paths=paths,
        error_label=f"git fetch {remote} --prune",
    )
    return f"fetched {remote} and pruned remote-tracking refs"
...
def main() -> None:
    ...
    parser.add_argument("--host", default=os.environ.get("CODEX_GITOPS_BIND", "127.0.0.1"))
    parser.add_argument("--port", type=int, default=int(os.environ.get("CODEX_GITOPS_PORT", "8765")))
