...
def _flutter_executable() -> str:
    configured = os.environ.get("FLUTTER_BIN", "").strip()
    if configured:
        return configured
    resolved = shutil.which("flutter")
    if resolved:
        return resolved
    raise BridgeError("flutter executable is not installed or not available on PATH.")
...
def _run_flutter(request: FlutterRunRequest) -> dict[str, Any]:
    ...
    try:
        process = subprocess.Popen(
            [_flutter_executable(), *request.argv],
            cwd=str(cwd),
...
app = FastAPI(title="sync-flutter-http", version="0.1.0")


@app.get("/healthz")
async def healthz() -> dict[str, Any]:
    return {"ok": True, "flutter_bin": _flutter_executable()}
