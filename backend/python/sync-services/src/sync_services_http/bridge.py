...
def _mutagen_file() -> Path:
    raw = os.environ.get("PARALLELS_SYNC_MUTAGEN_FILE")
    if raw:
        return Path(raw).expanduser().resolve()
...
def _host_home() -> Path:
    raw = os.environ.get("PARALLELS_SYNC_HOST_HOME")
    if raw:
        return Path(raw).expanduser().resolve()
...
def _virtual_home(host_home: Path) -> Path:
    raw = os.environ.get("PARALLELS_SYNC_VIRTUAL_HOME")
    if raw:
...
def import_gitops_modules() -> tuple[Any, Any]:
    codex_home = _codex_home()
    gitops_src = codex_home / "mcp" / "gitops" / "src"
    if not gitops_src.exists():
        raise BridgeError(f"gitops source not found: {gitops_src}")
    if str(gitops_src) not in sys.path:
        sys.path.insert(0, str(gitops_src))
    workflow_ops = importlib.import_module("gitops_mcp.workflow_ops")
    gitops_server = importlib.import_module("gitops_mcp.server")
    return workflow_ops, gitops_server
