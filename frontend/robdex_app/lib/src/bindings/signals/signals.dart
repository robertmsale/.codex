// ignore_for_file: type=lint, type=warning
// ignore_for_file: unused_import
library signals_types;

import 'dart:typed_data';
import 'package:meta/meta.dart';
import 'package:tuple/tuple.dart';
import '../serde/serde.dart';
import '../bincode/bincode.dart';

import 'dart:async';
import 'package:rinf/rinf.dart';

export '../serde/serde.dart';

part 'trait_helpers.dart';
part 'agent_runtime_action_availability.dart';
part 'agent_runtime_action_row.dart';
part 'agent_runtime_api_error.dart';
part 'agent_runtime_approval_card.dart';
part 'agent_runtime_badge.dart';
part 'agent_runtime_chat_entry.dart';
part 'agent_runtime_command_registry_decision_input.dart';
part 'agent_runtime_command_request_card.dart';
part 'agent_runtime_command_seed.dart';
part 'agent_runtime_controller_state.dart';
part 'agent_runtime_conversation_shell_view_model.dart';
part 'agent_runtime_discovery_view.dart';
part 'agent_runtime_fact.dart';
part 'agent_runtime_final_execution_policy.dart';
part 'agent_runtime_god_mode_state.dart';
part 'agent_runtime_gui_operation.dart';
part 'agent_runtime_managed_process_row.dart';
part 'agent_runtime_model_option.dart';
part 'agent_runtime_operation_result.dart';
part 'agent_runtime_operation_surface.dart';
part 'agent_runtime_output.dart';
part 'agent_runtime_output_signal.dart';
part 'agent_runtime_projection_snapshot.dart';
part 'agent_runtime_registry_scope.dart';
part 'agent_runtime_request.dart';
part 'agent_runtime_request_signal.dart';
part 'agent_runtime_requirement_input.dart';
part 'agent_runtime_requirements_review_panel.dart';
part 'agent_runtime_role_admin_view.dart';
part 'agent_runtime_role_detail.dart';
part 'agent_runtime_role_editor_draft.dart';
part 'agent_runtime_role_editor_draft_view.dart';
part 'agent_runtime_role_editor_lifecycle_authority_metadata.dart';
part 'agent_runtime_role_editor_model_defaults.dart';
part 'agent_runtime_role_editor_options_view.dart';
part 'agent_runtime_role_editor_routing_metadata.dart';
part 'agent_runtime_role_editor_visibility_metadata.dart';
part 'agent_runtime_role_policy_entry.dart';
part 'agent_runtime_role_policy_row.dart';
part 'agent_runtime_role_row.dart';
part 'agent_runtime_role_version_row.dart';
part 'agent_runtime_selected_session_control_plane.dart';
part 'agent_runtime_session_row.dart';
part 'agent_runtime_shell_project_row.dart';
part 'agent_runtime_shell_role_presentation.dart';
part 'agent_runtime_stream_outcome.dart';
part 'agent_runtime_timeline_row.dart';
part 'agent_runtime_workbench_view_model.dart';
part 'agent_runtime_workflow_memory_detail.dart';
part 'agent_runtime_workflow_memory_event.dart';
part 'agent_runtime_workflow_memory_feedback_payload.dart';
part 'agent_runtime_workflow_memory_row.dart';
part 'agent_runtime_workflow_memory_view.dart';
part 'archive_thread_group_signal.dart';
part 'archive_thread_signal.dart';
part 'bridge_task_result_signal.dart';
part 'clear_project_hook_logs_signal.dart';
part 'create_project_signal.dart';
part 'create_thread_group_signal.dart';
part 'create_thread_signal.dart';
part 'decide_approval_signal.dart';
part 'delete_project_signal.dart';
part 'delete_thread_group_signal.dart';
part 'fetch_thread_history_signal.dart';
part 'hook_toast_signal.dart';
part 'initialize_workbench_signal.dart';
part 'interrupt_thread_signal.dart';
part 'load_image_bytes_signal.dart';
part 'load_period_stats_signal.dart';
part 'load_project_hook_logs_signal.dart';
part 'load_requirement_composables_signal.dart';
part 'load_thread_stats_signal.dart';
part 'move_selected_thread_to_group_signal.dart';
part 'reload_workbench_signal.dart';
part 'rename_thread_group_signal.dart';
part 'rename_thread_signal.dart';
part 'select_project_signal.dart';
part 'select_thread_signal.dart';
part 'send_thread_message_signal.dart';
part 'set_project_orchestrator_signal.dart';
part 'set_thread_requirements_signal.dart';
part 'set_thread_running_state_signal.dart';
part 'spawn_agent_signal.dart';
part 'terminal_close_all_signal.dart';
part 'terminal_close_signal.dart';
part 'terminal_event_signal.dart';
part 'terminal_input_signal.dart';
part 'terminal_open_signal.dart';
part 'terminal_resize_signal.dart';
part 'terminate_command_execution_signal.dart';
part 'thread_compact_signal.dart';
part 'thread_history_state_signal.dart';
part 'update_global_settings_signal.dart';
part 'update_project_signal.dart';
part 'update_thread_settings_signal.dart';
part 'update_worker_metadata_signal.dart';
part 'upload_image_bytes_signal.dart';
part 'warm_handoff_signal.dart';
part 'workbench_diagnostics_signal.dart';
part 'workbench_selected_chat_delta_signal.dart';
part 'workbench_state_signal.dart';
part 'signal_handlers.dart';
